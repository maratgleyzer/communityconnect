...
<html>
    ...
    "groupData": [
		...
		//var _serviceAreas=["SM28","CH37","ood"];
		var _serviceAreas=["MT20"];
		var _serviceAreaQueryService = "https://www.gartrellgroup.net/ArcGIS/rest/services/CivicTechnologies/CommunityConnectSvc/MapServer/1";
		...
		*************/
		...
		//dojo.addOnLoad(init);
		dojo.addOnLoad(dojo.partial("init",_basemapMapService,_layersMapService,_blockgroupsQueryService,_legendInfoUrl, _legendService, _legendSoapUrl, _token,_legendtoken, _classificationData,_serviceAreaQueryService,_serviceAreas));
		
		function init(basemapMapService,layersMapService,blockgroupsQueryService, legendInfoUrl, legendService, legendSoapUrl, token, legendToken, classificationData, serviceAreaQueryService, serviceAreas) {		
			console.log("init");
			...
			// set extent to service areas if provided
			if(serviceAreas && serviceAreas.length>0)
				setExtent(serviceAreas,serviceAreaQueryService,token);
		...
		function setExtent(serviceAreas,serviceAreaQueryService,token)
		{
			//build query task
			queryTask = new esri.tasks.QueryTask(serviceAreaQueryService+"?token="+token);

			var queryString;
			for(var i=0;i<serviceAreas.length;i++)
			{
				if(i==0)
					queryString = "SA='"+serviceAreas[i]+"'";
				else
					queryString+=" or SA='"+serviceAreas[i]+"'";
			}
			...
			queryTask.execute(query, function(fset) {
				...
				map.setExtent(serviceExtent.expand(1.2),true);
		...
		function showFeatureSet(fset,classificationData) {
			...
			{
				var thisColor = new dojo.Color(classificationData.groupData[i].color);
				var thisColorRGB = thisColor.toRgb();
				thisColorRGB[3]=0.55;
				symbols[i] = new esri.symbol.SimpleFillSymbol(esri.symbol.SimpleFillSymbol.STYLE_SOLID, new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new dojo.Color([0,0,0]), 1), new dojo.Color(thisColorRGB));
			}
