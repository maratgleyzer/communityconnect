...
/*members FusionCharts, args, attributes, class, constructor, core,
    ...
    src, swfUrl, toString, uniqueId, version, width
*/
...
(function () {
    ...
    global.core = function (command) {
        ...
        this.src = options.swfUrl;
        if (!(this.src && typeof this.src.toString === 'function' &&
            this.src.toString().length > 0)) {
            global.raiseError(this, '25081843', 'param', '',
...
(function () {
     ...
     * @type void
     */
    var raiseEWEvent = function (sender, id, nature, source, message, level) {

        // Keep a reference to "message" parameter as, the param can be
        // optionally an error object.
        var msg = message;

        // If the message is sent as error, we throw it.
        if (message instanceof Error) {
            // Customize the error message.
            msg = message.message;

            message.name = errorNatures[nature];
            message.module = 'FusionCharts' + source;
            message.level = level;
            message.message = '#' + id + ' FusionCharts' + source + ' >> ' + msg;

            // Throw error in a separate scope sho that the execution of this script
            // is not blocked.
...
/*members FusionCharts, XMLURL, addEventListener, appendChild, apply,
    ...
    toLowerCase, toString, undefined, update
*/
    ...
    global.extend({
        render: function (containerElement) {
            ...
            if (isDuplicateId(this.id, containerElement)) {
                global.raiseError(new Error("#03091502 FusionCharts.render() " +
...
/*members ActiveXObject, FC_DataLoadError, FusionChartsDataFormats,
    XMLHttpRequest, transcodeData, abort, addDataHandler, addEventListener,
    ajax, apply, call, cancelDataLoad, cancelDataLoadRequest, core, data,
    dataError, dataFormat, dataSource, decode, encode, error, extend,
    format, get, getChartData, httpStatus, id, location, onreadystatechange,
    open, options, overrideMimeType, policies, protocol, prototype, push,
    raiseError, raiseEvent, readyState, renderer, responseText, send,
    sender, setChartData, setChartDataUrl, setRequestHeader, slice, source,
    status, test, toLowerCase, toString, update, url, x, chartData,
    xmlHttpRequestObject, cancelDataUpdate
*/
...
(function () {
     ...
     */
    var handlers = {}, dataStore = {}, xStore = {}, cache = {};
    ...
    global.extend({
        ...
        setChartData: function (data, format, silent, _recursed) {
            ...
            if (format === undefined || format === null ||
                typeof format.toString !== 'function') {
            ...
            if (/url$/i.test(format)) {
                ...
                if (_recursed !== true) {
                    // Method to cancel data-load request.
                    var cancelDLRFlag = false;
                    ...
                    if (xStore[this.id] && typeof xStore[this.id].abort === 'function') {
                        if (xStore[this.id].readyState && xStore[this.id].readyState !== 0) {
                            xStore[this.id].abort();
                        }
                    }

                    // Initiate XmlHttpRequest.
                    xStore[this.id] = global.ajax.get(decodeURIComponent(data), function (responseText, x) {
...
(function () {
    ...
    window.getChartFromId = function (id) {
        return global.core.items[id] === undefined ?
            global.swfobject.getObjectById(id) : global.core.items[id];
    };
...
/*members addEventListener, animation, chart, extend, getChartAttribute,
    getChartData, id, length, raiseError, replace, sender, graph,
    setChartAttribute, setChartData, test, toLowerCase, toString
*/
...
(function () {
    ...
    window.__fusioncharts_vars = function (id, vars) {
        ...
        if (typeof vars === 'object') {
            ...
            if (vars.dataURL !== undefined || vars.dataXML !== undefined) {
                obj.addEventListener('DataLoaded', syncDataStore);
            }
            // Specifically unescape the dataXML updated
            if (vars.dataXML !== undefined) {
                obj.flashVars.dataXML = window.unescape(vars.dataXML);
            }
            return true;
        }
    ...
    var renderer = {
        ...
        render: function (container, callBack) {
            ...
            encodedVars.dataXML = window.encodeURIComponent(this.flashVars.dataXML || '');
            encodedVars.dataURL = urlEncodeIfNecessary(this.flashVars.dataURL || '');
...
(function () {
    ...
    FCC.init = function () {
        ...
        var FCFCC = {
            ...
            method: {
                getExternalInterfaceMethods: function (chartType) {
                    ...
                    (FCFCC.methodCSV[chartType || this.chartType()] || '');
                },
        ...
        var formrtNumber = function (num, obj, type) {
            ...
            if (chart.formatnumber == 1) {
                ...
                result = (splitVal === undefined) || ((decimalVal) === 0) ?
                    formatedNum : (formatedNum) + decimalVal;
            }
        ...
        var addEIMethods = function (ref, obj) {
            ...
            // Store a reference to the chart swf HTML Node.
            var chartName = convertSWFtoAlias(obj.src);

            // The externalInterfaceMethods names are parsed from CSV to
            // Array.
            var eiItems = FCFCC.method.getExternalInterfaceMethods(chartName).split(','), i;
        ...
        var newFCCstub = function (cObj) {
            return {
                        ...
                        dataLabels : {
                            ...
                            formatter : function () {
                                ...
                                if (this.series.type == 'pie') {
                                    ...
                                    y = showvalue === '1' ? this.percentage + (cObj.chart.showpercentagevalues === '1' ? '%' : ''): '';
                ...
                yAxis: [{
                    ...
                    labels: {
                        formatter : function () {
                            return parseStr(formrtNumber(this.value, cObj.chart, 1));
                        }
                    },
                    plotBands: [],
                    plotLines: []
                    ...
                    labels: {
                        enabled : false,
                        formatter : function () {
                            return parseStr(formrtNumber(this.value, cObj.chart, 2));
                        }
                    },
                    opposite: true
                }],
        ...
        function addVline(hc, color, alpha, width, value) {
            hc.xAxis.plotLines.push({
                color: convertColor(color, alpha, '#444444', 100),
                width: width ? width : 1,
                value: value
            });
        }
        ...
        function addPoint(seriesObj, pointObj, series, chartName, id, HCObj, getScolor, bubbleRatio, FCObj) {
            ...
            seriesType = seriesObj.type || HCObj.chart.defaultSeriesType, dataseperator;

            //itterate through pointObj
            if (pointObj instanceof Array) {
                for (y = 0; y <= pointObj.length; y += 1) {
                    if (typeof pointObj[y] === 'object') {
                        if (!pointObj[y].vline) {
                            ...
                            // add the data point
                            seriesObj.data.push({
                                y: (isAbs ? Math.abs(parseFloat(pointObj[y].value || pointObj[y].y)):parseFloat(pointObj[y].value || pointObj[y].y)) || '',
                                ...
                                color: pointObj[y].color || series < 2 ? convertColor(pointObj[y].color || getScolor(y), pointObj[y].alpha || 100): undefined,
                                ...
                                marker: (seriesType === 'area' || seriesType === 'line') ? {
                                    ...
                                    fillColor: pointObj[y].anchorbgcolor ? convertColor(pointObj[y].anchorbgcolor, pointObj[y].anchorbgalpha, 'FFFFFF', 100) : undefined,
                                    lineColor: pointObj[y].anchorbordercolor ? convertColor(pointObj[y].anchorbordercolor, pointObj[y].anchoralpha, '767575', 100) : undefined,
                        ...
                        for (x = 0; x < value.length; x += 1) {
                            seriesObj.data.push({
                                y : value[x] ? (isAbs ? Math.abs(parseFloat(value[x])):parseFloat(value[x])) : ''
                            });
                        }
                    }
                }
...
///////function to add category
                    ...
                    if (obj.dataset[x].dataset && obj.dataset[x].dataset instanceof Array) {
                        for (y = 0; y < obj.dataset[x].dataset.length ; y += 1) {
                            newSeries = {
                                ...
                                data: [],
                                color: convertColor(obj.dataset[x].color, obj.dataset[x].dataset[y].alpha, getScolor(count_set), 100),
                            ...
                            if (obj.dataset[x].dataset[y].data instanceof Array) {
                                for (z = 0; z < max_cat; z += 1) {
                                    ...
                                    newSeries.data.push({
                                        ...
                                        id: id,
                                        color: obj.dataset[x].dataset[y].data[z].color ? convertColor(obj.dataset[x].dataset[y].data[z].color, obj.dataset[x].dataset[y].data[z].alpha, getScolor(count_set), 100) : undefined
                                    });
                                }
                            }
        ...
        function addData(obj, series, chartName, hc, width, height, id) {
            ...
            getScolor = function (index) {
                ...
                palett = (obj.chart.palettecolors) ? ((typeof palett === 'string') ? obj.chart.palettecolors.split(',') : obj.chart.palettecolors) : FCFCC.color;
            ...
            case 1.5:  //pareto
                ...
                if (series == 1.5) {
                    ...
                    if (obj.data instanceof Array) {
                     ...
                     ////to fixe secondary yaxis max 100% issue
                    hc.chart.alignTicks = false;
                    hc.yAxis[1].alternateGridColor = null;
                    hc.yAxis[1].gridLineWidth =  0;
                }
                ...
                if (series !== 6) {
                    if (obj.dataset) {
                        ...
                        for (x = 0;x < obj.dataset.length;x += 1) {
                            ...
                            z.marker = (seriesType === 'area' || seriesType === 'line') ? {
                                    ...
                                    fillColor: obj.dataset[x].anchorbgcolor ? convertColor(obj.dataset[x].anchorbgcolor, obj.dataset[x].anchorbgalpha, 'FFFFFF', 100) : undefined,
                                    lineColor: obj.dataset[x].anchorbordercolor ? convertColor(obj.dataset[x].anchorbordercolor, obj.dataset[x].anchoralpha, '767575', 100) : undefined,
            ...
            case 7.5:
                ...
                if (obj.dataset) {
                    count_set = 0;
                    for (x = 0; x < obj.dataset.length; x += 1) {
                        z = createSeries(series, convertColor(obj.dataset[x].color, obj.dataset[x].alpha, getScolor(count_set), 100), obj.dataset[x].renderas ? FCFCC.combi[obj.dataset[x].renderas]: ((obj.dataset[x].parentyaxis == 'S' && (series == 5 || series == 5.5)) ? 'line': undefined), obj.dataset[x].seriesname, obj.dataset[x].parentyaxis == 'S', obj.dataset[x].showvalues, undefined, undefined);
                        z.marker = {
 ...
 //function to convert a marimekko chart
                ...
                for (z = 0; z < HCObj.series[y].data.length ; z += 1) {
                    ...
                    if (catCountFlag) {
                        ...
                        if (FCObj.chart.showsum != '0') {
                            HCObj.labels.items.push(createText(formrtNumber(arr[z], FCObj.chart), '-14', temp));
                        }
                    }
        ...
        function covertToFCC(renderTo, chartname, obj, id, width, hight) {
            ...
            if (obj.chart.anchorsides) {
                hc.plotOptions.area.marker.symbol = hc.plotOptions.line.marker.symbol = convertAncorSide(parseInt(obj.chart.anchorsides, 10));
            }

            //////////////line option/////////////
            hc.plotOptions.line.lineWidth = parseInt(obj.chart.linethickness, 10) || (y === 1.5 ? 2 : 3);
            hc.plotOptions.line.color = convertColor(obj.chart.linecolor || '767575', obj.chart.linealpha || 100);

            //////////////area option/////////////
            hc.plotOptions.area.color = convertColor(obj.chart.plotfillcolor || '767575', obj.chart.plotfillalpha || 90);
            hc.plotOptions.area.lineColor  = convertColor(obj.chart.plotbordercolor || "333333", obj.chart.plotborderalpha || 95);
            hc.plotOptions.area.lineWidth   = parseInt(obj.chart.plotborderthickness, 10) || 1;
                        ...
                        for (z = 0; z < obj.vtrendlines[x].line.length; z += 1) {
                            if (obj.vtrendlines[x].line[z].istrendzone !== '0') {
                                hc.xAxis.plotBands.push({
                                    color: convertColor(obj.vtrendlines[x].line[z].color, obj.vtrendlines[x].line[z].alpha, '333333', 99),
            ...
            if (chartname == 'Marimekko') {
                ...
                hc.tooltip.formatter = function () {
                    ...
                    if (obj.chart.usepercentdistribution != '0') {
                        ...
                        x += formrtNumber(this.y, obj.chart, 1);
                    }
        ...
        FCC.render = function (__container, callBack) {
            ...
            if (FCC.isReady()) {
                ...
                HCJson.chart.events.load = function () {
                    ...
                    global.raiseEvent('dataloaded', {}, sender);
        ...
        var parse = {
            ...
            attr: function (attrObj) {
                ...
                for (i = 0; i < attrObj.length; i += 1) {
                    obj[attrObj[i].nodeName.toLowerCase()] =
                        attrObj[i].nodeValue;
                }
