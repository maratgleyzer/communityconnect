...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	// --- END SVG METHODS -----------------------------
		...
		public function Image_imagick($url, $x, $y, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='') {
			
			$tempname = tempnam(K_PATH_CACHE,'url_');
			...
			if (!isset($this->images[$file])) {
				...
				// ImageMagick handler function
				$img=new Imagick();
				$img->readImage($file);
				$img->resizeImage($neww,$newh,Imagick::FILTER_LANCZOS,1,false);
				$img->setImageFormat('bmp');
				$info = $this->_toJPEG_imagick($img);			
				$img->destroy(); 
		...
		* @access protected
		*/
		protected function _toJPEG_imagick($image) {
